...
class ProfileService {
  ...
  Future<Map<String, dynamic>?> loadCachedProfile(String uid) async {
    ...
    final cachedUsers = await DatabaseService.instance.getCachedCollection('users');
  ...
  Future<void> cacheChildren(String uid, List<Map<String, dynamic>> children) async {
    if (uid.isEmpty) return;
  ...
  Stream<Map<String, dynamic>> profileStream(String uid) {
    return _firestore.collection('users').doc(uid).snapshots().asyncMap((snapshot) async {
  ...
  Stream<List<Map<String, dynamic>>> childrenStream(String uid) {
    return _firestore
      ...
      // Сортируем на клиенте по createdAt (desc). Поля могут быть Timestamp, DateTime, int или ISO string.
      children.sort((a, b) {
        DateTime? toDateTime(dynamic v) {
          ...
          try {
            ...
            if (v.runtimeType.toString().contains('Timestamp')) {
              // Defensive call: try to call toDate()
              try {
  ...
  Stream<Map<String, dynamic>?> childStream(String childId) {
    ...
    return _firestore.collection('children').doc(childId).snapshots().map((snapshot) {
  ...
  Future<String> uploadChildPhoto(String uid, String childId, Uint8List bytes) async {
    if (uid.isEmpty || childId.isEmpty || bytes.isEmpty) {
      throw ArgumentError('Не указаны данные для загрузки фото ребёнка');
    }

    final storageRef = _storage.ref().child('child_photos').child(uid).child('$childId.jpg');
  ...
  Future<String> toggleChildRequestStatus(String uid, String childId, bool? currentRequestStatus) async {
    ...
    if (currentRequestStatus == true) {
      ...
      message = newRequestStatus == null ? 'Заявка отправлена' : 'Заявка отменена';
    }
  ...
  Future<Map<String, dynamic>?> loadCachedKindergartenInfo() async {
    return await DatabaseService.instance.getCachedDocument('kindergarten', 'info');
  }
